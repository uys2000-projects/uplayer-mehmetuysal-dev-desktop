<template>
  <div ref="container" class="flex flex-wrap overflow-y-auto h-full">
    <div ref="topBuffer" :style="{ height: topBufferSize + 'px', width: '100%' }"></div>
    <component :is="cellComponent" v-for="item in currentItems" v-bind="{ item: item, itemHeight, columnCount }"
      :key="item.id" />
